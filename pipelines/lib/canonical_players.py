"""Canonical player resolution: the ONE name -> player_key path for the repo bake.

Repo-owned port of football-signal/engine/canonical_players.py (2026-09-22).
The Supabase `players` table is the canonical roster. Every bake stage that
receives a player NAME from any source resolves it here, once, to the numeric
`player_key` -- and carries that key downstream. Joins happen on player_key,
never on re-derived name strings.

Differences from the workspace original (all deliberate, all repo-contained):
  - NICKNAMES comes from pipelines/lib/nicknames.py (vendored copy).
  - IDENTITY_SNAPSHOT is data/inputs/player_identity_map.json (vendored).
  - No ~/workspace sys.path hacks; sbclient is imported via the
    supabase-football-signal skill path, same pattern as
...
Usage:
    from lib.canonical_players import resolve, load_registry
...
class Registry:
    ...
    def lookup_dst(self, raw):
        ...
        # Stale/alternate team abbreviations (ESPN uses LAR/WSH; canonical is
        # LA/WAS). Normalize before token lookup.
        _DST_ABBR_FIX = {"lar": "la", "wsh": "was", "jac": "jax",
                         "stl": "la", "oak": "lv", "sd": "lac"}
        s = _DST_ABBR_FIX.get(s, s)
...
def resolve_skill(name, registry=None):
    ...
    yields a unique hit, else None (fail-closed).
    """
    reg = registry or load_registry()
    hits = set()
